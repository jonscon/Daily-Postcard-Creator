...
app.get("/", function (request, response) {
  response.sendFile(__dirname + '/public/bow-acute-brother-creator.html');
});
...
// server code for handling share Postcard post request
app.post('/sharePostcard', function(request, response) {
    console.log(request.body);
    let postcardObject = request.body;
    let JSONString = JSON.stringify(postcardObject);
    fs.writeFile('postcardData.json', JSONString, (err) => {
      if (err) {
          console.error(err);
          return;
      }
...
// handle the display page
app.get('/formatPostcard', function(request, response) {
  console.log("aight we good");
  fs.readFile('postcardData.json', 'utf8', (err, data) => {
    response.json(data);
  })
});
